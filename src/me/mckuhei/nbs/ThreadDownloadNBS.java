...
public class ThreadDownloadNBS extends Thread {
	...
	public void run() {
		try {
			player.sendMessage(Main.plugin.pluginPrefix+Main.plugin.config.getString("downloading"));
			URLConnection conn=this.url.openConnection();
			conn.setDoInput(true);
			conn.connect();
			NBS nbs=new NBS(new MyInputStream(conn.getInputStream()));
			player.sendMessage(Main.plugin.pluginPrefix+Main.plugin.config.getString("downloaded"));
